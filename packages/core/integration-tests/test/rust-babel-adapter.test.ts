...
describe('Rust Babel Adapter', function () {
  global.it('runs a basic babel plugin via the Rust engine', async function () {
    ...
    const fixturePath = path.join(__dirname, 'integration/babelrc-custom/index.js');
    ...
    assert(!file.includes('REPLACE_ME'), 'Babel plugin did not run: REPLACE_ME still present');
    assert(file.includes('hello there'), 'Babel plugin did not run: output does not contain "hello there"');
  });
});
