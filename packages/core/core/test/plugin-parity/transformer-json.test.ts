...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-json',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-json', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    const atlaspack = new Atlaspack({
      entries: path.join(FIXTURE_DIR, 'index.json'),
      mode: 'production',
      ...
      defaultTargetOptions: {
        ...
        shouldScopeHoist: false, // Not applicable for JSON
    ...
    // Find the asset for index.json
    let jsonAsset;
    bundleGraph.traverse((node) => {
      if (
        node.type === 'asset' &&
        path.basename(node.value.filePath) === 'index.json'
      ) {
        jsonAsset = node.value;
      }
    });

    assert(jsonAsset, 'Could not find index.json asset');

    // Get the code
    const code = await jsonAsset.getCode();

    // Get dependencies
    const dependencies = jsonAsset
      .getDependencies()
      .map((dep) => ({
