...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-inline',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-inline', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    bundleGraph.traverse((node) => {
      if (
        ...
        inlineAsset = node.value;
      }
    });

    assert(inlineAsset, 'Could not find index.txt asset');

    // Get the code
    const code = await inlineAsset.getCode();

    // Get dependencies
    const dependencies = inlineAsset
      .getDependencies()
      .map((dep) => ({
    ...
    // Get metadata
    const metadata = inlineAsset.meta;
