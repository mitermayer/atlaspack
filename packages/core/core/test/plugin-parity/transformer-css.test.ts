...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-css',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-css', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    const atlaspack = new Atlaspack({
      entries: path.join(FIXTURE_DIR, 'index.css'),
      mode: 'production',
      ...
      defaultTargetOptions: {
        ...
        shouldScopeHoist: false, // Not applicable for CSS
    ...
    // Find the asset for index.css
    let cssAsset;
    bundleGraph.traverse((node) => {
      if (
        node.type === 'asset' &&
        path.basename(node.value.filePath) === 'index.css'
      ) {
        cssAsset = node.value;
      }
    });

    assert(cssAsset, 'Could not find index.css asset');

    // Get the code
    const code = await cssAsset.getCode();

    // Get dependencies
    const dependencies = cssAsset
      .getDependencies()
      .map((dep) => ({
