...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-image',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-image', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    const atlaspack = new Atlaspack({
      entries: path.join(FIXTURE_DIR, 'test.png'),
      mode: 'production',
      ...
      defaultTargetOptions: {
        ...
        shouldScopeHoist: false, // Not applicable for images
    ...
    // Find the asset for test.png
    let imageAsset;
    bundleGraph.traverse((node) => {
      if (
        node.type === 'asset' &&
        path.basename(node.value.filePath) === 'test.png'
      ) {
        imageAsset = node.value;
      }
    });

    assert(imageAsset, 'Could not find test.png asset');

    // Get the code (for images, this might be base64 or a URL)
    const code = await imageAsset.getCode();

    // Get dependencies
    const dependencies = imageAsset
      .getDependencies()
      .map((dep) => ({
    ...
    // Get metadata
    const metadata = imageAsset.meta;
