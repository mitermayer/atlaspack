...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-tokens',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-tokens', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    const atlaspack = new Atlaspack({
      entries: path.join(FIXTURE_DIR, 'index.tokens.json'),
      mode: 'production',
      ...
      defaultTargetOptions: {
        ...
        shouldScopeHoist: false, // Not applicable for tokens
    ...
    // Find the asset for index.tokens.json
    let tokensAsset;
    bundleGraph.traverse((node) => {
      if (
        node.type === 'asset' &&
        path.basename(node.value.filePath) === 'index.tokens.json'
      ) {
        tokensAsset = node.value;
      }
    });

    assert(tokensAsset, 'Could not find index.tokens.json asset');

    // Get the code
    const code = await tokensAsset.getCode();

    // Get dependencies
    const dependencies = tokensAsset
      .getDependencies()
      .map((dep) => ({
    ...
    // Get metadata
    const metadata = tokensAsset.meta;
