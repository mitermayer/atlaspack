...
const FIXTURE_DIR = path.join(
  __dirname,
  '../__fixtures__/plugin-parity/transformer-yaml',
);
const GOLDEN_PATH = path.join(FIXTURE_DIR, 'golden.json');
const PROJECT_ROOT = path.resolve(__dirname, '../../../../../');

describe('plugin parity: transformer-yaml', function () {
  this.timeout(50000);
  ...
  it('matches golden output', async () => {
    ...
    const atlaspack = new Atlaspack({
      entries: path.join(FIXTURE_DIR, 'index.yaml'),
      mode: 'production',
      ...
      defaultTargetOptions: {
        ...
        shouldScopeHoist: false, // Not applicable for YAML
    ...
    // Find the asset for index.yaml
    let yamlAsset;
    bundleGraph.traverse((node) => {
      if (
        node.type === 'asset' &&
        path.basename(node.value.filePath) === 'index.yaml'
      ) {
        yamlAsset = node.value;
      }
    });

    assert(yamlAsset, 'Could not find index.yaml asset');

    // Get the code
    const code = await yamlAsset.getCode();

    // Get dependencies
    const dependencies = yamlAsset
      .getDependencies()
      .map((dep) => ({
