...
- Emit summary report with per-fixture status and exit non-zero on diff failures; document commands/paths for integration CI to invoke (no CI wiring in this repo).

## Acceptance Criteria

- Local command: `yarn atlaspack build --engine=dual <fixture>` (or documented equivalent) produces parity report and artifacts for both engines in predictable paths.
- Diff report includes: bundle hash match/mismatch, graph structure diff, diagnostics diff summary; timings recorded; commands/paths documented for integration CI to call.
