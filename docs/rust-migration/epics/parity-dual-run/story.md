...
- Reports stored under `.parcel-cache/parity/<engine>/<fixture>/` with merged diff summary per run.

## Acceptance Criteria

- Dual-run executable locally and in CI via flag/env; errors include guidance to rerun single-engine fallback.
...
- Smoke suite completes <10m locally; full suite available for nightly CI.
